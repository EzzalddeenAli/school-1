...
class MailSmsHandler {
	...
	public function sms($to,$message){
		...
		}elseif($this->settings['smsProvider'] == "enterpriseliveair"){
			$this->enterpriseliveair($to,$message);
	...
	public function concepto($to,$message){
		if($this->settings['conceptoUserName'] == "" || $this->settings['conceptoPassword'] == "" || $this->settings['conceptoSenderId'] == "") return;
		$to = $this->prepareNumberFormat($to,"00");
		$sess = file_get_contents("http://premium.liveair.in/httpapi/httpapi?token=".$this->settings['conceptoUserName']."&sender=".$this->settings['conceptoSenderId']."&number=".$to."&route=2&type=1&sms=".urlencode($message));
		if(Config::get('app.debug') == true){
			Log::info($sess);
		}
	}
	...
	public function custom($to,$message){
		...
		$this->settings['customHTTPParams'] = str_replace("{message}",trim($message),$this->settings['customHTTPParams']);
	...
	public function enterpriseliveair($to,$message){
		if($this->settings['enterpriseliveairUserName'] == "" || $this->settings['enterpriseliveairPassword'] == "") return;
		$sess = file_get_contents("http://enterprise.liveair.in/httpapi/httpapi?token=".$this->settings['enterpriseliveairUserName']."&sender=".$this->settings['enterpriseliveairSenderId']."&number=".$to."&route=2&type=1&sms=".urlencode($message));
		if(Config::get('app.debug') == true){
			Log::info($sess);
		}
	}
