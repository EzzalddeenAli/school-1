...
class StudentsController extends \BaseController {
	...
	public function gsearch() {
		...
		// print_r($_GET['filter']['filters'][0]['value']);die;
		$page = 1;
		$keyword = $_GET['filter']['filters'][0]['value'];
		if (strpos($keyword, '||') !== false) {
			$keyword = explode("||", $keyword);
			$searchQ = $keyword[0];
			$searchC = $keyword[1];
		...
		if ($this->data['users']->role == "parent") {
			...
			if (count($studentId) > 0) {
				...
				$toReturn['totalItems'] = $students->count();
				$students = $students->orderBy('studentRollId', 'ASC')->get()->toArray();
			} else {
				$students = array();
				$toReturn['totalItems'] = 0;
			}
		...
		$studentsData['data'] = array();
		while (list(, $student) = each($students)) {
			$studentsData['data'][] = array('id' => $student['id'], "studentRollId" => $student['studentRollId'], "fullName" => $student['fullName'], "username" => $student['username'], "email" => $student['email'], "isLeaderBoard" => $student['isLeaderBoard'], "studentClass" => isset($classArray[$student['studentClass']]) ? $classArray[$student['studentClass']] : "");
		}
		$studentsData['count'] = $total_student;
		return $studentsData;
	}
	...
	function marksheetBulkPDF() {
		...
		foreach ($users as $student) {
			...
			<table cellspacing='0' style='padding: 1px; width: 100%; font-size: 11pt; '>
	            <tr>
	                <th style='width: 100%; text-align: center; '> Marksheet </th>
	            </tr>
			</table>
			<br/>
			<table cellspacing='5' style='width: 100%; font-size: 12px;'>
			...
			if (!isset($studentMarks[Input::get('examId')]['data'])) {
				...
				$content .= "<table cellspacing='0' style='padding: 0px;margin:0px; width: 100%; border: solid 1px black; '>
		                <tbody><tr>
		                    ...
		                    <th style='width:20%;border: solid 1px #000000; padding:2px;'>Attendance marks</th>
		                    <th style='width:7%;border: solid 1px #000000; padding:2px;'>Mark</th>
		                    <th style='width:7%;border: solid 1px #000000; padding:2px;'>Grade</th>
							<th style='width:15%;border: solid 1px #000000; padding:2px;'>Pass grade</th>
		                    <th style='width:15%;border: solid 1px #000000; padding:2px;'>Final grade</th>
		                    <th style='width:10%;border: solid 1px #000000; padding:2px;'>Status</th>
				...
				$content .= "<br/><br/>
					<table cellspacing='0' style='padding: 1px; width: 100%; font-size: 10pt; '>
			            <tr>
			                <th style='width: 100%; text-align: center; '> Exam marks : " . $studentMarks[Input::get('examId')]['totalMarks'] . " - Average points : " . $studentMarks[Input::get('examId')]['pointsAvg'] . " </th>
			            </tr>
					</table>
					<br/><br/>";
			}
